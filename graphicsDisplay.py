...
class InfoPane:
  ...
  def drawPane(self):
    self.scoreText_0 = text( self.toScreen(0, 0  ), self.textColor, "SCORE:    0", "Times", self.fontSize, "bold")
    self.scoreText_1 = text( self.toScreen(200, 0  ), GHOST_COLORS[2], "SCORE:    0", "Times", self.fontSize, "bold")
    self.timeText = text( self.toScreen(420, 0  ), GHOST_COLORS[3], "TIME: ", "Times", self.fontSize, "bold") 
  ...
  def updateScore(self, score,index=0):
    if index==0:
      changeText(self.scoreText_0, "SCORE: % 4d" % score)
    else:
      changeText(self.scoreText_1, "SCORE: % 4d" % score)
  
  def updateTime(self, time):
    changeText(self.timeText, "TIME: % 4d" % time)
...
class PacmanGraphics:
  ...
  def update(self, newState,timepassed):
    ...
    if agentState.isPacman:
      self.animatePacman(agentState, prevState, prevImage,agentIndex)
    else:
    ...
    self.infoPane.updateScore(newState.score[agentIndex],agentIndex)
    self.infoPane.updateTime(timepassed)
  ...
  def drawPacman(self, pacman, index):
    ...
    print("draw pacman")
    if index ==0 and pacman.scaredTimer == 0:
      fillColor = PACMAN_COLOR #yellow
      outlineColor = PACMAN_COLOR
      print("yellow not scared")
    elif index ==1 and pacman.scaredTimer == 0:
      fillColor = GHOST_COLORS[2] #orange
      outlineColor = GHOST_COLORS[2]
      print("orange not scared")

    else:
      print("scared")
      fillColor = SCARED_COLOR
      width = SCARED_OUTLINE_WIDTH
      if index ==0:
        outlineColor = PACMAN_COLOR
      else:
        outlineColor = GHOST_COLORS[2]
  ...
  def movePacman(self, pacman, position, direction, image,agentIndex = 0):
    screenPosition = self.to_screen(position)
    endpoints = self.getEndpoints( direction, position )
    r = PACMAN_SCALE * self.gridSize
    if pacman.scaredTimer > 0:
      color = SCARED_COLOR
      # outlineColor = SCARED_COLOR
      # print("scared")
      width = SCARED_OUTLINE_WIDTH
      if agentIndex ==0:
        outlineColor = PACMAN_COLOR
      ...
      if agentIndex ==0:
        color = PACMAN_COLOR #yellow
        outlineColor = PACMAN_COLOR
        # print("yellow not scared")
      elif agentIndex ==1:
        color = GHOST_COLORS[2] #orange
        outlineColor = GHOST_COLORS[2]
        # print("orange not scared")
    edit(image[0], ('fill', color), ('outline', outlineColor))
    moveCircle(image[0], screenPosition, r, endpoints)
    refresh()

  def animatePacman(self, pacman, prevPacman, image,agentIndex = 0):
    if self.frameTime < 0:
    ...
    if self.frameTime > 0.01 or self.frameTime < 0:
      ...
      for i in range(1,int(frames) + 1):
        ...
        self.movePacman(pacman,pos, self.getDirection(pacman), image,agentIndex)
        refresh()
        sleep(abs(self.frameTime) / frames)
    else:
      self.movePacman(pacman,self.getPosition(pacman), self.getDirection(pacman), image,agentIndex)
    refresh()
